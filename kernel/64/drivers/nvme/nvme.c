    ...
    if (Specialty == PCI_DEVICE_NVME)
    {
        ...
        while (CapPointer != 0)
        {
            ...
            if (PCI_Read8(Path, CapPointer) == 0x11)
            {
                ...
                MSIX_VectorControl* msixTableBase = AllocVMAtPhys(msixTableBase0, 0x5000);
        ...
        if ((Header.BAR0 & 0b110) == 0b100) AllocMap(Controller.vBar0, ((uint64_t)Header.BAR0 & 0xFFFFFFF0) | ((uint64_t)Header.BAR1 << 32), 0x20000, (1 << MALLOC_READWRITE_BIT) | (1 << MALLOC_CACHE_DISABLE_BIT));
        else AllocMap(Controller.vBar0, ((uint64_t)Header.BAR0 & 0xFFFFFFF0), 0x20000, (1 << MALLOC_READWRITE_BIT) | (1 << MALLOC_CACHE_DISABLE_BIT));
